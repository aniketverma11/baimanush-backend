...
class PostMixinManager(models.Manager):
    def all(self, *args, **kwargs):
        return super(PostMixinManager, self).filter(is_active=True, publish__lte=timezone.now(), is_deleted=False)

    def active(self, *args, **kwargs):
        return super(PostMixinManager, self).filter(is_active=True, is_deleted=False)

    def filter(self, *args, **kwargs):
        return super(PostMixinManager, self).filter(is_active=True, is_deleted=False).filter(*args, **kwargs)
